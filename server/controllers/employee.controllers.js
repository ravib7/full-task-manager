...
exports.getAllTodos = async (req, res) => {
    try {                                               // from protect
        ...
        console.log(error)
        res.status(500).json({ message: "unable to get all todos" })
    }
}
...
exports.toggleTodoStatus = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "toggle todo status success" })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "unable to toggle todo status" })
    }
}
